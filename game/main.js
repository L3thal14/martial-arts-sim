...
let imagePath = (frameNumber, animation) => {
  return (
    "/martial-arts-sim/game/images/" + animation + "/" + frameNumber + ".png"
  );
};
...
loadImages((images) => {
  ...
  document.addEventListener("keypress", (event) => {
    const key = event.key;
    if (key === "d") queuedAnimations.push("punch");
    else if (key === "w") queuedAnimations.push("kick");
    else if (key === "x") queuedAnimations.push("forward");
    else if (key === "a") queuedAnimations.push("backward");
    else if (key === "s") queuedAnimations.push("block");
  });
});
